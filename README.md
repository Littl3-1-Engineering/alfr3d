...
- **Context Exchange with the Launcher**: ALFR3D and the Nexus Launcher each run their own situational-awareness engine, so each context facet is owned by whichever side has the better evidence for it and the other subscribes instead of re-deriving. Part-of-day is backend-owned (only ALFR3D knows the household's real Morning/Bedtime routine rows) and served by `GET /api/context/day-context`; the launcher's own device state — battery, charging, Do Not Disturb, headset, network, orientation — is launcher-owned and reported back via `POST /api/context/device-snapshot`, stored per device so two launchers can disagree. Every facet carries provenance and a freshness stamp, and an unreachable peer degrades to a local estimate rather than to no context. First consumer: `check_focus_needed()` drops its "find a quiet spot" advice when a launcher reports Do Not Disturb is already on. See `todo/todo_context_exchange_protocol.md`.
...
- **Time-of-day awareness**: `common/day_context.py` derives one `DayContext` (waking hours, part-of-day, greeting, a pre-bed "wind-down" window) from the `environment` row plus the Sunrise/Morning/Sunset/Bedtime routines. It backs the speak mute gate, the LLM prompt, the arrival greeting, the daemon's idle-quip timer (which now goes quiet ~45 min before the Bedtime routine), and — via `GET /api/context/day-context` — the Nexus Launcher, which subscribes to this answer rather than re-deriving part-of-day from its own fixed clock buckets.
