...
class TestPruneCardInteractions:
    """Tests for prune_card_interactions() -- card_interactions had no retention mechanism at
    all before 2026-09-13 (todo_sa_data_retention_3yr.md)."""

    @patch("services.service_daemon.alfr3ddaemon.pymysql.connect")
    def test_deletes_rows_older_than_the_retention_cutoff(self, mock_connect):
        from services.service_daemon.alfr3ddaemon import prune_card_interactions
        ...
        prune_card_interactions()

        delete_calls = [
            call
            for call in mock_cursor.execute.call_args_list
            if "DELETE FROM card_interactions" in call.args[0]
        ]
        assert len(delete_calls) == 1
        mock_db.commit.assert_called_once()
        mock_db.close.assert_called_once()
    ...
    def test_default_retention_is_the_shared_two_year_default(self):
        from services.service_daemon.alfr3ddaemon import (
            CARD_INTERACTIONS_RETENTION_DAYS,
            SA_EVENT_LOG_RETENTION_DAYS_DEFAULT,
        )

        assert SA_EVENT_LOG_RETENTION_DAYS_DEFAULT == 730
        assert CARD_INTERACTIONS_RETENTION_DAYS == SA_EVENT_LOG_RETENTION_DAYS_DEFAULT


class TestRecordSaStorageMetrics:
    """Tests for record_sa_storage_metrics() -- the real, queryable growth-tracking history
    behind todo_sa_data_retention_3yr.md's projections."""

    @patch("services.service_daemon.alfr3ddaemon.pymysql.connect")
    def test_inserts_one_row_per_tracked_table(self, mock_connect):
        from services.service_daemon.alfr3ddaemon import (
            SA_STORAGE_METRICS_TRACKED_TABLES,
            record_sa_storage_metrics,
        )
        ...
        mock_connect.return_value = mock_db
        mock_db.cursor.return_value = mock_cursor
        mock_cursor.fetchone.return_value = (100, 4096, 2048)
        ...
        insert_calls = [
            call
            for call in mock_cursor.execute.call_args_list
            if "INSERT INTO sa_storage_metrics" in call.args[0]
        ]
        assert len(insert_calls) == len(SA_STORAGE_METRICS_TRACKED_TABLES)
        mock_db.commit.assert_called_once()

    @patch("services.service_daemon.alfr3ddaemon.pymysql.connect")
    def test_skips_a_table_information_schema_cant_find_this_cycle(self, mock_connect):
        """A renamed/dropped table (or a transient information_schema miss) must not abort the
        whole snapshot -- the other tracked tables should still get their row this cycle."""
        from services.service_daemon.alfr3ddaemon import (
            SA_STORAGE_METRICS_TRACKED_TABLES,
            record_sa_storage_metrics,
        )
        ...
        mock_connect.return_value = mock_db
        mock_db.cursor.return_value = mock_cursor
        mock_cursor.fetchone.side_effect = [None] + [(1, 1, 1)] * (
            len(SA_STORAGE_METRICS_TRACKED_TABLES) - 1
        )
        ...
        assert len(insert_calls) == len(SA_STORAGE_METRICS_TRACKED_TABLES) - 1

    @patch("services.service_daemon.alfr3ddaemon.pymysql.connect")
    def test_rolls_back_and_closes_on_db_error(self, mock_connect):
        import pymysql
        from services.service_daemon.alfr3ddaemon import record_sa_storage_metrics
