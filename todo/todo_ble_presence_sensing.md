...
## Status: 🔴 Dead at Phase 0 — re-verified 2026-09-16, same result, no stably-identifiable
personal device found in this household's real BLE environment

Third item of Wave 3, following SA-6 (in progress) and SA-9 (Phase 0/1 shipped 2026-09-16).
Hardware-gated by design — the task doc's own framing explicitly welcomes a "dead" outcome here
as a legitimate result, not a failure to force through.

**This doc was previously deleted** (commit `3282a3c9`, `docs(todo): remove 24 fully-completed
todos`) — a mislabeling; "dead at Phase 0" is not "completed," and the finding below still
applies today. Restored from git history (`git show a2989c9b:todo/todo_ble_presence_sensing.md`)
with a 2026-09-16 re-verification appended. `todo/todo_next_session.md`'s SA-8 bullet, which still
pointed at this filename, is accurate again now that the file exists.

## Re-verification, 2026-09-16

Prompted by moving on to SA-8 as the next Wave 3 item after SA-9 shipped, and by the ~2.5-week
gap plus a new device (the ESPHome sensor) having just joined the household the same day — worth
checking whether the BLE landscape changed too, per this doc's own stated re-open condition.

Same methodology as the original Phase 0 pass, against the same real production hardware
(`alfr3d@192.168.2.200`): adapter re-confirmed LE-capable (`hciconfig hci0 features` still shows
`<LE support>`/`<LE and BR/EDR>`; `btmgmt info` still shows `le` in current settings), then two
independent ~30-second passive scans (`bluetoothctl scan on` + `devices` readback) about 3 minutes
apart.

- **Same result.** 30 and 25 distinct devices per scan. The **same eleven** named/stable devices
  resolved in both scans, identical MAC + name pair each time: `Govee_H6199_276F`, three TVs
  (`[TV] Samsung Q60AA 65 TV`, `[TV] Moonrise TV`, `[TV] Samsung 7 Series (55)`), `65" Crystal
  UHD`, `[LG] webOS TV OLED55C2PUA`, `Furbo3-S3`, `Venus_EC64C928886E`, `JBL Go 3`, and a generic
  `net` (likely a network appliance advertising BLE, not personal). All fixed household
  infrastructure — same category as the original finding, still nothing personally-carried.
- **New this time: `athom-tem-hum-4a4734` (`8C:FD:49:4A:47:36`)** — SA-9's own ESPHome sensor,
  which turns out to also broadcast BLE (most ESP32 modules do) alongside the WiFi/API integration
  SA-9 actually uses. Resolved identically in both scans. Still fixed infrastructure, not a
  presence signal — noted only because it's a genuinely new stable device since the last pass, for
  completeness, not because it changes the verdict.
- **Anonymous-placeholder devices again dominate** (~19-25 per scan) and again show partial churn
  between the two scans — roughly half the MACs recurred across the ~3-minute gap, half didn't.
  This is consistent with, not contradictory to, standard BLE privacy address rotation (typical
  10-15 minute rotation intervals mean a ~3-minute gap will often catch the same still-unrotated
  address twice) — not evidence of a newly-stable personal device. No anonymous MAC resolved to a
  real name in either scan.
- **No wearable, headphone, fitness-tracker, or phone-type device resolved to a stable identity in
  either scan.** The original verdict stands unchanged.

**Verdict: still Red — dead.** Nothing has changed in this household's real BLE environment that
would make presence sensing from personally-carried devices feasible. Re-check again only if a
BLE wearable/tracker that deliberately broadcasts a fixed, resolvable identity actually enters the
household (per the original re-open condition below) — not on a fixed schedule.
...
- **A longer-window test** (hours, not minutes) of whether slower-rotating addresses (like the
  original pass's `A0:36:BC:8D:89:5C`) ever stabilise long enough to be useful, or whether they're
  on the same short rotation as everything else and this pass's short window just hadn't caught
  the next rotation yet. Not pursued given the sample size of "zero named personal devices" (now
  confirmed twice, ~2.5 weeks apart) already clears the doc's own "stop here" bar without needing
  to resolve this.
